...
def print_word_list(list, letter)
...
def print_words_array(array)
  ...
  array.each_with_index do |list, index|
    ...
    unless list.nil?
      count = print_word_list(list, letter)
      total = total + count
  ...
  puts "\nThere were #{total} unique words in the file."
  puts "The highest word count was #{highest_count}."
  puts "\nLetter(s) that began words #{highest_count} times were"
  most_common_letters.each { |letter| puts "\t'#{letter}'/'#{letter.upcase}'" }
end

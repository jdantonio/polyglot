...
void PrintWorkLists(WorkLists&);
    int PrintWordList(WordList&);
void PrintElapsedTime(clock_t, clock_t);
...
void FreeWorkLists(WorkLists& list) {
  ...
  WordNode* nextNode = NULL;      // Pointer to next node in list
  ...
  for (int i = 0; i < WORK_LIST_ARRAY_SIZE; i++) {
    ...
    // iterate over all nodes in the current list
...
void PrintElapsedTime(clock_t start, clock_t stop) {
  ...
  int diff = (stop - start) * 1000.0 / ((double) CLOCKS_PER_SEC);

  std::cout << "Elapsed time: " << diff << " msecs ("
            << ticks << " ticks)" << endl;
}

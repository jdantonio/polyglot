...
(def all-words (read-words-from-file filename))
(def total-words (count all-words))
(def unique-words (set (validate-and-filter-words all-words)))
(def total-unique-words (count unique-words))
(def unique-words-by-letter (words-by-first-letter unique-words))

; print aggregate data
(println (str "Results for file " filename ":  " total-words " total words processed.\n"))

; print per-letter data and word lists
(def highest-word-count (apply max (map count unique-words-by-letter)))
